...
final class ApexAnalyzerSettingsPanel {
    ...
    ApexAnalyzerSettingsPanel() {
        ...
        importButton.addActionListener(e -> ImportFromSonarQubeAction.runImport(null, () ->
                loadFrom(ApexAnalyzerSettings.getInstance())));
    ...
    private void applyFilter(String filter) {
        ...
        for (RuleRow r : visible) {
            ApexAnalyzerSettings.RuleOverride override = settings.rules.get(r.key);
    ...
    void loadFrom(@NotNull ApexAnalyzerSettings settings) {
    ...
    void saveTo(@NotNull ApexAnalyzerSettings settings) {
        ...
        Map<String, ApexAnalyzerSettings.RuleOverride> existingRules = new java.util.HashMap<>(settings.rules);
        Map<String, ApexAnalyzerSettings.RuleOverride> rules = settings.rules;
        rules.clear();
        if (settings.rulesReplaceDefaults) {
            for (RuleRow r : allRows) {
                RowState state = tableState.get(r.key);
                ApexAnalyzerSettings.RuleOverride existing = existingRules.get(r.key);
                boolean enabled;
                String severity;
                if (state != null) {
                ...
                rules.put(r.key, withExistingParams(new ApexAnalyzerSettings.RuleOverride(enabled, severity), existing));
            }
        } else {
            for (RuleRow r : allRows) {
                RowState state = tableState.get(r.key);
    ...
    private static ApexAnalyzerSettings.RuleOverride withExistingParams(
            ApexAnalyzerSettings.RuleOverride next,
            ApexAnalyzerSettings.RuleOverride existing) {
